...
class invoice_edit(models.Model):
    ...
    oci_ac_numerobiexterne = fields.Char(string="N° BI externe")
    ...
    oci_ac_typeintervention = fields.Selection(
        [('DEPANNAGE', 'DEPANNAGE'),
         ('SOUS GARANTIE', 'SOUS GARANTIE'),
         ('INSTALLATION NEUVE', 'INSTALLATION NEUVE'),
         ('INSTALLATION OCCASION', 'INSTALLATION OCCASION'),
         ('LIVRAISON', 'LIVRAISON'),
         ('MAINTENANCE', 'MAINTENANCE'),
         ('MISE EN SERVICE', 'MISE EN SERVICE'),
         ('SAV', 'SAV')], string="Type Intervention")
    oci_ac_nomintervention = fields.Char(string="Tâche")
...
class invoice_create(models.TransientModel):
    ...
    def _prepare_invoice_values(self, order, name, amount, so_line):
        ...
        res['oci_ac_numerobi'] = order.oci_so_numerobi
        res['oci_ac_numerobiexterne'] = order.oci_so_numerobiexterne
        res['oci_ac_techniciens'] = order.oci_so_techniciens
        res['oci_ac_dateintervention'] = order.oci_so_dateintervention
        res['oci_ac_typeintervention'] = order.oci_so_typeintervention
        return res
...
class invoice_add(models.TransientModel):
    ...
    oci_so_numerobi = fields.Char(string="N° BI")
    oci_so_numerobiexterne = fields.Char(string="N° BI externe")
    oci_so_techniciens = fields.Many2many('hr.employee', string="Technicien(s)")
    oci_so_dateintervention = fields.Date(string="Date d'Intervention")
    oci_so_typeintervention = fields.Selection(
        [('DEPANNAGE', 'DEPANNAGE'),
         ('SOUS GARANTIE', 'SOUS GARANTIE'),
         ('INSTALLATION NEUVE', 'INSTALLATION NEUVE'),
         ('INSTALLATION OCCASION', 'INSTALLATION OCCASION'),
         ...
         ('MISE EN SERVICE', 'MISE EN SERVICE'),
         ('SAV', 'SAV')],    string="Type Intervention")
    oci_so_representant = fields.Many2one('hr.employee', string="Représentant")

    def _prepare_invoice(self):
        res = super()._prepare_invoice()
        if self.oci_so_numerobi:
            res['oci_ac_numerobi'] = self.oci_so_numerobi
            if self.oci_so_numerobiexterne:
                res['oci_ac_numerobi'] = self.oci_so_numerobiexterne
        if self.oci_so_techniciens:
            res['oci_ac_techniciens'] = self.oci_so_techniciens
        if self.oci_so_dateintervention:
            res['oci_ac_dateintervention'] = self.oci_so_dateintervention
        if self.oci_so_typeintervention:
            res['oci_ac_typeintervention'] = self.oci_so_typeintervention
        return res
